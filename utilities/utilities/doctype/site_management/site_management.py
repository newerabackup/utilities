...
class SiteManagement(Document):
    ...
    if TYPE_CHECKING:
        ...
        action: DF.Literal["Create Site", "Migrate", "Backup", "Restore", "Drop Site","Maintenance On","Maintenance Off"]
    ...
    def execute_bench_command(self, action, site_name, domain=None, backup_file=None):
        commands = {
            ...
            "Maintenance_on": f"bench --site {site_name} set-config maintenance_mode 1",
            "Maintenance_off": f"bench --site {site_name} set-config maintenance_mode 0",
        }
    ...
    def maintenance_on(self, site_name):
        self.execute_bench_command("Maintenance_on", site_name)
        
    def maintenance_off(self, site_name):
        self.execute_bench_command("Maintenance_off", site_name)
    ...
    def before_save(self):
        ...
        elif self.action == "Maintenance On":
            self.maintenance_on(self.site_name)
        elif self.action == "Maintenance Off":
            self.maintenance_off(self.site_name)
